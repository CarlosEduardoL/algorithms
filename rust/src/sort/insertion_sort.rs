...
///
/// Insertion sort is a simple algorithm that sorts a list of items by repeatedly inserting each item
/// into its correct position within the sorted portion of the list. To do this, we compare the current
/// item with the items to its left until we find its correct position, then we insert it. This process
/// is repeated for each item in the list until they are all in their correct positions.
///
/// This implementation also includes a check to see if the list is already sorted, which could avoid
